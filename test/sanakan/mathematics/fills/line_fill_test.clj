...
(def matched (lf/select-line (first (first lined)) (second lined) negative-decider-fn))
(fact matched => (l/line (p/point -5 -4) (p/point -1 -4)))

;; Test that growing a segment works.
(def matched2 (lf/grow-cluster (list (first (first lined))) (second lined) negative-decider-fn))
...
(def parts (lf/clusters (p/point -5 -5) 5 5 blocked-decider-fn))
(fact (count      parts)    => 3)
...
(def parts2 (lf/clusters (p/point 0 0) 5 5 complex-decider-fn))
(fact (count      parts2)      => 3)
(fact (count (nth parts2 0))   => 4)
(fact (nth   (nth parts2 0) 0) => (l/line (p/point 3 5) (p/point 5 5)))
(fact (nth   (nth parts2 0) 1) => (l/line (p/point 3 4) (p/point 5 4)))
(fact (nth   (nth parts2 0) 2) => (l/line (p/point 3 3) (p/point 5 3)))
(fact (nth   (nth parts2 0) 3) => (l/line (p/point 3 2) (p/point 5 2)))
...
(fact (count (nth parts2 2))   => 6)
(fact (nth   (nth parts2 2) 0) => (l/line (p/point 0 5) (p/point 1 5)))
(fact (nth   (nth parts2 2) 1) => (l/line (p/point 0 4) (p/point 1 4)))
(fact (nth   (nth parts2 2) 2) => (l/line (p/point 0 3) (p/point 1 3)))
(fact (nth   (nth parts2 2) 3) => (l/line (p/point 0 2) (p/point 1 2)))
(fact (nth   (nth parts2 2) 4) => (l/line (p/point 0 1) (p/point 5 1)))
(fact (nth   (nth parts2 2) 5) => (l/line (p/point 0 0) (p/point 5 0)))

;; Test that partitioning works when clusters end
(def parts3 (lf/clusters (p/point -5 -5) 5 5 vertical-decider-fn))
(fact (count      parts3)      => 3)
(fact (count (nth parts3 0))   => 1)
(fact (nth   (nth parts3 0) 0) => (l/line (p/point -5  2) (p/point 5  2)))
(fact (count (nth parts3 1))   => 7)
(fact (nth   (nth parts3 1) 0) => (l/line (p/point -5  1) (p/point 5  1)))
(fact (nth   (nth parts3 1) 1) => (l/line (p/point -5  0) (p/point 5  0)))
(fact (nth   (nth parts3 1) 2) => (l/line (p/point -5 -1) (p/point 5 -1)))
(fact (nth   (nth parts3 1) 3) => (l/line (p/point -5 -2) (p/point 5 -2)))
(fact (nth   (nth parts3 1) 4) => (l/line (p/point -5 -3) (p/point 5 -3)))
(fact (nth   (nth parts3 1) 5) => (l/line (p/point -5 -4) (p/point 5 -4)))
(fact (nth   (nth parts3 1) 6) => (l/line (p/point -5 -5) (p/point 5 -5)))
...
(fact (lf/cluster-size (nth parts3 0)) => 11)
(fact (lf/cluster-size (nth parts3 1)) => 77)
...
(fact (lf/in-cluster? (p/point   0   0) (nth parts3 1)) => true)

(fact (lf/line-weight (nth (nth parts3 1) 0)) => {:wx 0 :wy 11})
(fact (lf/line-weight (nth (nth parts3 0) 0)) => {:wx 0 :wy 22})
(fact (lf/line-weight (nth (nth parts2 1) 0)) => {:wx 2 :wy 5})
(fact (lf/line-weight (nth (nth parts2 0) 0)) => {:wx 12 :wy 15})
...
(fact (lf/cluster-center (nth parts2 0)) => (p/point 4  3))
