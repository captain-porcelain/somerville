...
(deftest splitting
  (let [cl (ch/cut-line points)
    ...
    (is (= p4  (:p1 (:line (nth sa 0)))))
    (is (= p1  (:p2 (:line (nth sa 0)))))
    (is (= p1  (:p1 (:line (nth sa 1)))))
    (is (= p3  (:p2 (:line (nth sa 1)))))
    (is (= p3  (:p1 (:line (nth sa 2)))))
    (is (= p8  (:p2 (:line (nth sa 2)))))
    (is (= p8  (:p1 (:line (nth sb 0)))))
    (is (= p11 (:p2 (:line (nth sb 0)))))
    (is (= p11 (:p1 (:line (nth sb 1)))))
    (is (= p9  (:p2 (:line (nth sb 1)))))
    (is (= p9  (:p1 (:line (nth sb 2)))))
    (is (= p4  (:p2 (:line (nth sb 2)))))))

(deftest hull
  (let [h (ch/quick-hull points)]
    (is (= p4  (:p1 (nth (:lines h) 0))))
    (is (= p1  (:p2 (nth (:lines h) 0))))
    (is (= p1  (:p1 (nth (:lines h) 1))))
    (is (= p3  (:p2 (nth (:lines h) 1))))
    (is (= p3  (:p1 (nth (:lines h) 2))))
    (is (= p8  (:p2 (nth (:lines h) 2))))
    (is (= p8  (:p1 (nth (:lines h) 3))))
    (is (= p11 (:p2 (nth (:lines h) 3))))
    (is (= p11 (:p1 (nth (:lines h) 4))))
    (is (= p9  (:p2 (nth (:lines h) 4))))
    (is (= p9  (:p1 (nth (:lines h) 5))))
    (is (= p4  (:p2 (nth (:lines h) 5))))))
