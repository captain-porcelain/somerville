...
(deftest outline-graph
  (let [pa1 (p/point 2 3)
        ...
        ;a (p/average (list pa1 pa2 pa3 pb1 pb2 pb3 pb4))
        ;og (poly/outline-graph p1 p2)
        ;tmp (dorun (println "average:"))
        ;tmp (dorun (println (commons/out a)))
        ...
        ;tmp (dorun (println (clojure.string/join "\n" (map #(p/angle pll a %) (get og pll)))))
        ;tmp (dorun (println "outline graph:"))
        ]
    (dorun (println (poly/outline p1 p2)))))
