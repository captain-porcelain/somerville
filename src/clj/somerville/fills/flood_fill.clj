...
(defn fill
  [seed decider-fn x1 y1 x2 y2]
  (loop [frontier #{seed}
         visited #{}
         accepted #{}]
    (if (= 0 (count frontier))
      accepted
      (let [reachable (filter #(decider-fn seed %) frontier)
    ...
    (loop [i 0
      ...
      (if (= i sample-size)
        ...
        (let [seed (p/point (rand-int (+ x1 (- x2 x1))) (rand-int (+ y1 (- y2 y1))))
              part (if (= 0 (count (filter #(some #{seed} %) partitions)))
                     (fill seed decider-fn x1 y1 x2 y2)
                     #{})]
          (recur (+ i 1) (conj partitions part)))))))
