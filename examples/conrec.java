    ...
    public void contour(double [][] d, int ilb, int iub, int jlb, int jub, double [] x, double [] y, int nc, double [] z) {
        ...
        for (j=(jub-1);j>=jlb;j--) {
            for (i=ilb;i<=iub-1;i++) {
				...
				//----- Get max and min values of 4 points
				...
				//----- check if min and max are somewhere inside the defined hight lines
                if (dmax>=z[0]&&dmin<=z[nc-1]) {

                    for (k=0;k<nc;k++) {

						//----- check if min and max are relevant for current hightline
                        if (z[k] >= dmin && z[k] <= dmax) {
                            ...
                            for (m=4;m>=0;m--) {
								...
								//------ Now we have the vertices for the 4 triangles

								//------ This is used to decide which case each triangle has
                                if (h[m] > 0.0) {
                                    sh[m] = 1;
                                } else if (h[m] < 0.0) {
                                    sh[m] = -1;
